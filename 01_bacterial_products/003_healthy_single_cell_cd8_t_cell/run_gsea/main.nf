...
process de_analysis {
  ...
  output:
    ...
    val paired_grp, emit: paired_grp   // Emit the paired group name
  ...
  script:
    ...
    def condcol = contrast.CONDCOL != null ? contrast.CONDCOL : ( params.condition_col != "" ? "${params.condition_col}" : '') //
    paired_grp = contrast.PAIREDGRP != null ? "--paired_grp=${contrast.PAIREDGRP}" : ( params.paired_grp != "" ? "--paired_grp=${params.paired_grp}" : '') // Include paired group info if specified
    ...
    def merge_duplicate_genes = params.merge_duplicate_genes ? "--merge_duplicate_genes" : ''

    """
    mkdir -p ${out_dir}
    runDESeq2_ICBI.R ${sample_sheet} ${raw_counts} \\
      --result_dir=${out_dir} \\
      --c1=${test} \\
      --c2=${ref} \\
...
process generate_report {
    ...
    script:
      ...
      paired_grp = paired_grp != "" ? "--paired_grp=${paired_grp}" : ""
    ...
    module purge
    module load texlive
    Report.R ${analysis} ${contrast_name} ${bib_file}  ${template_file} \\
      ${paired_grp} \\
      ${remove_batch_effect} \\
      ${batch_col} \\
      ${covariate_formula}
...
workflow {
    ...
    report_template_file = file(workflow.projectDir + "/deseq2icbi/report_template.tex", checkIfExists:true) // Path to report template
    ...
    report_out = generate_report(deseq2_out.deseq2_res_dir, deseq2_out.contrast, deseq2_out.paired_grp, deseq2_out.deseq2_subdir, citation_out.bib_file, report_template_file) // Run the `generate_report` process
}
